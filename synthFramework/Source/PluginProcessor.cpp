...
void JuceSynthFrameworkAudioProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
    ...
    for (int i = 0; i < mySynth.getNumVoices(); i++)
    {
        ...
        if ((myVoice = dynamic_cast<SynthVoice*>(mySynth.getVoice(i))))
        {
            ...
            myVoice->getEnvelopeParams(attack, release, sustain, decay);
            
            float*  wavetype = (float*) tree.getRawParameterValue("wavetype");
            myVoice->getOscType(wavetype);
            
            float*  wavetype2 = (float*) tree.getRawParameterValue("wavetype2");
            myVoice->getOsc2Type(wavetype2);
            
            
            float*  filterType = (float*) tree.getRawParameterValue("filterType");
            float*  filterCutoff = (float*) tree.getRawParameterValue("filterCutoff");
            float*  filterRes = (float*) tree.getRawParameterValue("filterRes");
            
            myVoice->getFilterParams(filterType, filterCutoff, filterRes);

            float* mastergain = (float*) tree.getRawParameterValue("mastergain");
            float* blend = (float*) tree.getRawParameterValue("blend");
            float* pbup = (float*) tree.getRawParameterValue("pbup");
            float* pbdown = (float*) tree.getRawParameterValue("pbdown");
